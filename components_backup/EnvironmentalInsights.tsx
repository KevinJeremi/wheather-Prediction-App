...
export function EnvironmentalInsights({ 
  ...
  const getAirQualityLevel = (value: number) => {
    if (value <= 50) return { level: 'Good', color: 'green', bgColor: 'bg-green-100 dark:bg-green-900/20' };
    if (value <= 100) return { level: 'Moderate', color: 'yellow', bgColor: 'bg-yellow-100 dark:bg-yellow-900/20' };
    if (value <= 150) return { level: 'Unhealthy for Sensitive', color: 'orange', bgColor: 'bg-orange-100 dark:bg-orange-900/20' };
    return { level: 'Unhealthy', color: 'red', bgColor: 'bg-red-100 dark:bg-red-900/20' };
  };
      ...
      <div className="space-y-4">
        ...
        <div className={`p-3 rounded-xl ${airQualityInfo.bgColor} border border-white/20 dark:border-gray-700/20`}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Air Quality</span>
            <span className={`text-xs font-medium text-${airQualityInfo.color}-600 dark:text-${airQualityInfo.color}-400`}>
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-white/50 rounded-full h-2 overflow-hidden">
              <div 
                className={`h-full bg-gradient-to-r from-${airQualityInfo.color}-400 to-${airQualityInfo.color}-600`}
                style={{ width: `${Math.min(100, (airQuality / 200) * 100)}%` }}
              />
            </div>
        ...
        <div className={`p-3 rounded-xl ${uvInfo.bgColor} border border-white/20 dark:border-gray-700/20`}>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className={`text-xs font-medium text-${uvInfo.color}-600 dark:text-${uvInfo.color}-400`}>
              {uvInfo.level}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-white/50 rounded-full h-2 overflow-hidden">
              <div 
                className={`h-full bg-gradient-to-r from-${uvInfo.color}-400 to-${uvInfo.color}-600`}
