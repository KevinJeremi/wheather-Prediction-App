  ...
  return (
    <WeatherProvider value={contextValue}>
      <div className={isDark ? 'dark' : ''} suppressHydrationWarning>
          ...
          {!isDesktop && sidebarOpen && (
            <>
              ...
              <motion.div
                ...
                animate={{ x: 0 }}
                exit={{ x: -300 }}
                className="fixed left-0 top-0 h-full w-72 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl z-50 p-6 overflow-y-auto"
              >
                ...
                <nav className="space-y-2">
                  {[
                    { label: 'Home', value: 'home' as const, icon: '🏠', hasSubmenu: false },
                    { label: 'History', value: 'history' as const, icon: '📈', hasSubmenu: true },
                    { label: 'Alerts', value: 'alerts' as const, icon: '🔔', hasSubmenu: false },
                    { label: 'Settings', value: 'settings' as const, icon: '⚙️', hasSubmenu: false },
                  ].map((item) => (
                    <div key={item.value}>
                      <button
                        onClick={() => {
                          if (item.hasSubmenu) {
                            setMobileHistoryExpanded(!mobileHistoryExpanded);
                          } else {
                        ...
                        className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-xl transition-all ${(activeView === item.value || (item.hasSubmenu && ['weather-history', 'weather-prediction'].includes(activeView)))
                      ...
                      >
                        ...
                        {item.hasSubmenu && (
                          <motion.div
                            animate={{ rotate: mobileHistoryExpanded ? 180 : 0 }}
                        ...
                        <AnimatePresence>
                          {mobileHistoryExpanded && (
                            <motion.div
                              ...
                              transition={{ duration: 0.3 }}
                              className="overflow-hidden mt-1 ml-2 space-y-1"
                            >
                              {[
                                ...
                                <motion.button
                                  key={submenu.value}
                                  onClick={() => {
                                    setActiveView(submenu.value);
                                    setSidebarOpen(false);
                                    setMobileHistoryExpanded(false);
                                  }}
                                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all text-sm ${activeView === submenu.value
        ...
        {/* Main Content */}
        <main
          className={`max-w-full mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-200 ${isDesktop ? 'pt-8 pb-8' : 'pt-20 pb-8'
            }`}
          style={{
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {weatherData && (
                    <AIReasoning
                      ...
                      temperature={weatherData.hourly[0].temperature}
                      humidity={weatherData.hourly[0].humidity}
                      windSpeed={weatherData.hourly[0].windSpeed}
                      pressure={weatherData.hourly[0].pressure}
                      condition={getWeatherConditionText(weatherData.hourly[0].weatherCode)}
                      precipitation={weatherData.daily[0].precipitationSum}
